	...
	if [ -d $dir ] && [ "$dir" != "$TEST_DIR" ]; then
		...
		if [ $exitcode = 0 ]; then
			...
			for subdir in `ls $TEST_DIR`; do
				...
				# Runs the test.
				$TEST_DIR/$subdir/$TEST_SCRIPT "$subdir" "$dir" "$srcdir" "$TEMP_DIR" "$BASE_DIR" "$TEST_DIR/$subdir" "$SUBDIR_INPUT" "$SUBDIR_OUTPUT" "diff -y --strip-trailing-cr --suppress-common-lines --ignore-all-space --ignore-blank-lines -W 200"
