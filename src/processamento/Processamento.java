...
public class Processamento {
    ...
    public Processamento(HashMap<Integer, Candidato> candidatos, LinkedList<Partido> partidos, Date dataEleicao,
            int tipoEleicao) {
        ...
        Collections.sort(candidatosList, new Comparator<Candidato>() {
            ...
            public int compare(Candidato c1, Candidato c2) {
                if (c2.getVotos() != c1.getVotos())
                    ...
                    return c1.getDataNascimento().compareTo(c2.getDataNascimento()); // retorna < 0 se a data de
                                                                                     // nascimento de c1 for anterior a
                                                                                     // de c2, e vice-versa
            }
        });
    }
    ...
    public void ordenaPartidos() {
        ...
        for (Partido partido : partidos) {
            Collections.sort(partido.getCandidatos(), new Comparator<Candidato>() { // ordena os candidatos de cada
                                                                                    // partido
                @Override
                public int compare(Candidato c1, Candidato c2) {
                    if (c2.getVotos() != c1.getVotos())
    ...
    public void rel2() {
        ...
        for (Candidato candidatoEleito : candidatosEleitos) {
            ...
            if (candidatoEleito.getNumeroFederacao() == -1)
                System.out.print(i + " - " + candidatoEleito.getNome().toUpperCase() + " ("
                        + candidatoEleito.getPartido().getSigla() + ", "
                        + df.format(candidatoEleito.getVotos()).replaceAll(",", ".")); // substitui a vírgula por ponto,
                                                                                       // para tranformar algo no
                                                                                       // formato 1,156,145 em 1.156.145

            else
                System.out.print(i + " - *" + candidatoEleito.getNome().toUpperCase() + " ("
                        + candidatoEleito.getPartido().getSigla() + ", "
                        + df.format(candidatoEleito.getVotos()).replaceAll(",", "."));
    ...
    public void rel3() {
        ...
        for (Candidato candidato : candidatosList) {
            ...
            if (candidato.getNumeroFederacao() == -1)
                System.out
                        .print(i + " - " + candidato.getNome().toUpperCase() + " (" + candidato.getPartido().getSigla()
                                + ", " + df.format(candidato.getVotos()).replaceAll(",", "."));
    ...
    public void rel4() {
        ...
        for (Candidato candidato : candidatosList) {
            if (!candidatosEleitos.contains(candidato)) {
                if (candidato.getNumeroFederacao() == -1)
                    System.out.print(
                            i + " - " + candidato.getNome().toUpperCase() + " (" + candidato.getPartido().getSigla()
                                    + ", " + df.format(candidato.getVotos()).replaceAll(",", "."));
                else
                    System.out.print(
                            i + " - *" + candidato.getNome().toUpperCase() + " (" + candidato.getPartido().getSigla()
            ...
            i++;
            if (i == candidatosEleitos.size() + 1)
                break; // para não imprimir mais do que o número de vagas
        }
    }
    ...
    public void rel5() {
        ...
        for (Candidato candidatoEleito : candidatosEleitos) {
            if (!candidatosMaisVotados.contains(candidatoEleito)) { // se o candidato eleito não está no ranking de mais
                                                                    // votados
                if (candidatoEleito.getNumeroFederacao() == -1)
                    System.out.print(candidatosList.indexOf(candidatoEleito) + 1 + " - "
                            + candidatoEleito.getNome().toUpperCase() + " (" + candidatoEleito.getPartido().getSigla()
                            + ", " + df.format(candidatoEleito.getVotos()).replaceAll(",", "."));
                else
                    System.out.print(candidatosList.indexOf(candidatoEleito) + 1 + " - *"
                            + candidatoEleito.getNome().toUpperCase() + " (" + candidatoEleito.getPartido().getSigla()
    ...
    public void rel6() {
        ...
        for (Partido partido : partidos) {
            System.out.print(i + " - " + partido.getSigla() + " - " + partido.getNumero() + ", "
                    + df.format(partido.getVotosTotais()).replaceAll(",", "."));
            ...
            if (j > 1)
                System.out.println(j + " candidatos eleitos");
            else
                System.out.println(j + " candidato eleito");
            i++;
        }
    }
    ...
    public void rel8() {
        ...
        for (Candidato candidato : candidatosMaisVotados) {
            System.out.print(i + " - " + candidato.getPartido().getSigla() + " - " + candidato.getPartido().getNumero()
                    + ", " + candidato.getNome().toUpperCase() + " (" + candidato.getNumeroCandidato() + ", "
                    + df.format(candidato.getVotos()).replaceAll(",", "."));
            if (candidato.getVotos() > 1) {
                System.out.print(" votos)");
            } else {
            ...
            System.out.print(" / " + candidato.getPartido().getCandidatos().getLast().getNome().toUpperCase() + " ("
                    + candidato.getPartido().getCandidatos().getLast().getNumeroCandidato() + ", "
                    + df.format(candidato.getPartido().getCandidatos().getLast().getVotos()).replaceAll(",", "."));
    ...
    public void rel9() {
        ...
        System.out.println(
                "      Idade < 30: " + menorQue30 + " (" + nf.format(porcentagemMenorQue30).replace('.', ',') + "%)");
        System.out.println(
                "30 <= Idade < 40: " + menorQue40 + " (" + nf.format(porcentagemMenorQue40).replace('.', ',') + "%)");
        System.out.println(
                "40 <= Idade < 50: " + menorQue50 + " (" + nf.format(porcentagemMenorQue50).replace('.', ',') + "%)");
        System.out.println(
                "50 <= Idade < 60: " + menorQue60 + " (" + nf.format(porcentagemMenorQue60).replace('.', ',') + "%)");
        System.out.println(
                "60 <= Idade     : " + maiorQue60 + " (" + nf.format(porcentagemMaiorQue60).replace('.', ',') + "%)");
    }
    ...
    public void rel10() {
        ...
        System.out.println("Feminino: " + qtdFeminino + " ("
                + nf.format((float) qtdFeminino / candidatosEleitos.size() * 100).replace('.', ',') + "%)");
        System.out.println("Masculino: " + qtdMasculino + " ("
                + nf.format((float) qtdMasculino / candidatosEleitos.size() * 100).replace('.', ',') + "%)");
    }
    ...
    public void rel11() {
        ...
        System.out.println(" ("
                + nf.format((float) votosNominais / (votosNominais + votosLegenda) * 100).replace('.', ',') + "%)");
        System.out.print("Total de votos de legenda: " + df.format(votosLegenda).replaceAll(",", "."));
        System.out.println(
                " (" + nf.format((float) votosLegenda / (votosNominais + votosLegenda) * 100).replace('.', ',') + "%)");
    }
}
